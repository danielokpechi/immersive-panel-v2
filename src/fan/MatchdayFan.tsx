      ...
      <div style={colStyle}>
        ...
        <div style={phoneStyle}>
          ...
          {st.toast && (
            <div style={s('position:absolute;left:0;right:0;bottom:64px;z-index:55;display:flex;justify-content:center;pointer-events:none')}>
              <div style={s('display:flex;align-items:center;gap:9px;padding:11px 15px;background:var(--panel);animation:bgToast 2.4s ease both')}>
                <span style={s("font:800 17px/1 'KippaxCondensed','Archivo Black';color:#6CABDD")}>{st.toastXp}</span>
                <span style={s("font:700 11.5px/1 'Kippax','Archivo';letter-spacing:.06em;color:var(--on-panel)")}>{st.toast}</span>
              </div>
            </div>
          )}
          ...
          <div style={s('position:absolute;bottom:0;left:0;right:0;height:52px;z-index:40;display:flex;align-items:center;justify-content:center;padding:0 20px 12px;background:linear-gradient(transparent,var(--ground) 44%)')}>
            <div style={s("display:flex;align-items:center;gap:8px;padding:9px 15px;background:var(--sand);border-radius:100px;font:600 11.5px/1 'Kippax','Archivo';color:var(--body)")}><Ms size={13} color="var(--label)">lock</Ms>matchday.mancity.com</div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
          ...
          <div style={s('padding:22px 16px 0')}>
            ...
            <button onClick={go('reads')} className="fs" style={s('display:flex;align-items:stretch;width:100%;text-align:left;margin-top:9px;background:var(--sand);border-radius:2px;overflow:hidden;box-shadow:0 4px 16px rgba(0,24,56,.07)')}>
              <span style={s('flex:1;min-width:0;padding:14px 14px 15px')}>
                ...
                <span style={s("display:inline-flex;align-items:center;gap:6px;margin-top:11px;font:800 9.5px/1 'Kippax','Archivo';letter-spacing:.1em;color:var(--on-panel);background:var(--panel);padding:9px 10px")}><Ms size={13} color="var(--on-panel)">play_circle</Ms>WATCH HIGHLIGHTS</span>
              </span>
              <span style={s('width:116px;flex:none;position:relative;background:var(--sand2)')}><Slot id="last-meeting" label="First leg" /></span>
            </button>
          </div>
          <div style={s("padding:20px 16px 0;font:500 13.5px/1.5 'Kippax','Archivo';color:var(--body)")}>Tonight’s companion: follow the game, talk to other City fans, and earn towards a seat upgrade while you watch. Nothing to install.</div>
          <div style={s('padding:22px 16px 0')}>
            ...
            <div style={s('margin-top:6px')}>
              {[
                ...
                <div key={i} style={s('display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1.5px solid var(--hair)')}>
                  <span style={s('width:38px;flex:none;text-align:center')}><span style={s("display:block;font:800 17px/1 'KippaxCondensed','Archivo Black';color:var(--ink)")}>{x.day}</span><span style={s("display:block;font:800 8.5px/1 'Kippax','Archivo';letter-spacing:.1em;color:var(--label);margin-top:4px")}>{x.mon}</span></span>
                  <span style={s('flex:1;min-width:0')}><span style={s("display:block;font:700 13.5px/1.2 'Kippax','Archivo';color:var(--ink)")}>{x.opp}</span><span style={s("display:block;font:500 11.5px/1.3 'Kippax','Archivo';color:var(--label);margin-top:3px")}>{x.comp} · {x.ko}</span></span>
                  <span style={{ ...s("font:800 9px/1 'Kippax','Archivo';letter-spacing:.1em;padding:6px 7px"), color: x.fg, background: x.bg }}>{x.tag}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={s('margin-top:10px')}>
              {[{ label: 'POSSESSION', h: '58%', a: '42%', hf: 58, af: 42 }, { label: 'SHOTS', h: '17', a: '11', hf: 17, af: 11 }, { label: 'ON TARGET', h: '8', a: '4', hf: 8, af: 4 }, { label: 'EXPECTED GOALS', h: '2.34', a: '1.02', hf: 234, af: 102 }, { label: 'PASSES', h: '641', a: '498', hf: 641, af: 498 }, { label: 'CORNERS', h: '9', a: '3', hf: 9, af: 3 }].map((x) => (
                <div key={x.label} style={s('padding:10px 0;border-bottom:1.5px solid var(--hair)')}>
                  <div style={s('display:flex;align-items:baseline;justify-content:space-between;gap:10px')}><span style={s("font:800 17px/1 'Kippax','Archivo';color:var(--ink);font-variant-numeric:tabular-nums;width:52px")}>{x.h}</span><span style={s("font:800 9.5px/1 'Kippax','Archivo';letter-spacing:.14em;color:var(--label)")}>{x.label}</span><span style={s("font:800 17px/1 'Kippax','Archivo';color:var(--ink);font-variant-numeric:tabular-nums;width:52px;text-align:right")}>{x.a}</span></div>
          ...
          <div style={s('padding:22px 16px 0')}>
            ...
            <div style={s('display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:11px')}>
              <button onClick={go('photos')} className="fs" style={s('grid-column:span 2;text-align:left;background:var(--sand)')}>
                ...
                <div style={s('padding:12px 13px 13px')}><div style={s("font:800 22px/.94 'KippaxCondensed','Archivo Black';color:var(--ink)")}>The night in photos</div><div style={s("font:500 12.5px/1.45 'Kippax','Archivo';color:var(--body);margin-top:4px")}>1,204 fan photos from all four stands.</div></div>
              </button>
              <button onClick={go('reactions')} className="fs" style={s('text-align:left;background:var(--sand)')}><div style={s('height:96px;position:relative')}><Slot id="ft-reel" label="Reel still" /></div><div style={s('padding:11px 12px 12px')}><div style={s("font:800 19px/1 'KippaxCondensed','Archivo Black';color:var(--ink)")}>Reactions</div><div style={s("font:500 12px/1.4 'Kippax','Archivo';color:var(--body);margin-top:3px")}>30-second fan reels.</div></div></button>
              <button onClick={go('reads')} className="fs" style={s('text-align:left;background:var(--sand)')}><div style={s('height:96px;display:flex;align-items:center;justify-content:center')}><Ms size={34} color="var(--ink)">article</Ms></div><div style={s('padding:11px 12px 12px')}><div style={s("font:800 19px/1 'KippaxCondensed','Archivo Black';color:var(--ink)")}>Reads</div><div style={s("font:500 12px/1.4 'Kippax','Archivo';color:var(--body);margin-top:3px")}>Ratings and the tactical read.</div></div></button>
            </div>
          </div>
        </section>
      )}
            ...
            <div style={{ ...s('position:absolute;left:12px;right:12px;bottom:11px;display:flex;flex-direction:column;gap:8px;transition:transform .5s cubic-bezier(.2,.85,.2,1)'), transform: `translateY(${st.rollY}px)` }}>
              {previewMsgs.map((m, i) => (
                <div key={i} style={s('display:flex;gap:8px;align-items:flex-start')}>
                  <span style={{ ...s("width:22px;height:22px;flex:none;border-radius:50%;font:800 8.5px/22px 'Kippax','Archivo';text-align:center"), background: m.avBg, color: m.avFg }}>{m.initials}</span>
                  <span style={s('flex:1;min-width:0;background:var(--ground);padding:8px 10px;border-radius:2px 12px 12px 12px')}><span style={s("display:block;font:800 9px/1 'Kippax','Archivo';letter-spacing:.08em;color:var(--label)")}>{m.user}</span><span style={s("display:block;font:500 12.5px/1.35 'Kippax','Archivo';color:var(--ink);margin-top:4px")}>{m.text}</span></span>
                </div>
              ))}
            </div>
      ...
      <section style={s('margin:24px 0 0;padding:0 16px;animation:bgRise .4s .16s ease both')}>
        ...
        <div style={s("font:700 10.5px/1 'Kippax','Archivo';letter-spacing:.12em;color:var(--label);margin:16px 0 0;text-align:center")}>MATCHDAY.MANCITY.COM · NO APP, NO DOWNLOAD</div>
      </section>
    </div>
  );
}
        ...
        {shown.slice(-16).map((m: any) => {
          ...
          if (m.ev) return (
            <div key={m.id} style={{ ...s('display:flex;align-items:center;gap:10px;align-self:center;max-width:92%;padding:9px 13px;border-radius:100px;background:var(--sand)'), boxShadow: `inset 0 0 0 1.5px ${m.ev.color}44` }}>
              ...
              <span style={{ ...s("font:800 10px/1 'Kippax','Archivo';letter-spacing:.08em"), color: m.ev.color }}>{m.ev.kind}</span>
              <span style={s("font:600 12px/1.2 'Kippax','Archivo';color:var(--ink)")}>{m.text}{m.ev.minute ? ` · ${m.ev.minute}′` : ''}</span>
            </div>
          );
          ...
          return (
            <div key={m.id} style={{ ...s('display:flex;gap:8px;animation:bgBubble .3s ease both'), justifyContent: me ? 'flex-end' : 'flex-start' }}>
              {!me && <span style={{ ...s("width:28px;height:28px;flex:none;border-radius:50%;font:800 9.5px/28px 'Kippax','Archivo';text-align:center;margin-top:12px"), background: TEAM[m.t].bg, color: TEAM[m.t].fg }}>{m.user.slice(0, 2)}</span>}
...
function Predictions({ st, set, award }: any) {
  ...
  return (
    <div style={s('animation:bgFade .25s ease both;padding:18px 16px 28px')}>
      <div style={s("font:800 30px/.92 'KippaxCondensed','Archivo Black';color:var(--ink)")}>Call it before<br />kick-off</div>
      <div style={s("font:500 13px/1.5 'Kippax','Archivo';color:var(--body);margin-top:8px")}>Correct score pays 60 XP. First scorer pays another 60. Locks at the first whistle.</div>
      {!st.pred.done && (
        <div style={s('margin-top:18px;background:var(--sand);padding:16px')}>
          <div style={s("font:800 9.5px/1 'Kippax','Archivo';letter-spacing:.18em;color:var(--label)")}>CORRECT SCORE</div>
          <div style={s('display:flex;align-items:center;justify-content:center;gap:14px;margin-top:14px')}>
            {[['MCI', 'h'], ['RMA', 'a']].map(([lab, k], idx) => (
              <div key={lab as string} style={{ ...s('text-align:center'), order: idx === 1 ? 3 : 1 }}>
                <div style={s("font:800 10px/1 'Kippax','Archivo';letter-spacing:.1em;color:var(--body)")}>{lab}</div>
                <div style={s('display:flex;align-items:center;gap:8px;margin-top:8px')}>
                  <button onClick={() => set((sp: FSt) => ({ pred: { ...sp.pred, [k as string]: Math.max((sp.pred as any)[k as string] - 1, 0) } }))} className="fw" style={s("width:30px;height:30px;background:var(--ground);font:800 14px/1 'Kippax','Archivo';color:var(--ink)")}>−</button>
                  <span style={s("font:800 42px/.8 'KippaxCondensed','Archivo Black';color:var(--ink);width:34px;font-variant-numeric:tabular-nums")}>{(st.pred as any)[k as string]}</span>
                  <button onClick={() => set((sp: FSt) => ({ pred: { ...sp.pred, [k as string]: Math.min((sp.pred as any)[k as string] + 1, 9) } }))} className="fw" style={s("width:30px;height:30px;background:var(--ground);font:800 14px/1 'Kippax','Archivo';color:var(--ink)")}>+</button>
                </div>
              </div>
            ))}
          ...
          <button onClick={() => { set((sp: FSt) => ({ pred: { ...sp.pred, done: true } })); award(120, 'PREDICTION LOCKED', 'predict'); }} className="fp" style={s("display:block;width:100%;margin-top:18px;padding:15px;background:var(--panel);font:800 12.5px/1 'Kippax','Archivo';letter-spacing:.08em;color:var(--on-panel)")}>LOCK IN PREDICTION · 120 XP</button>
        </div>
      )}
      {st.pred.done && (
      ...
      <div style={s('margin-top:20px')}><div style={s("font:800 9.5px/1 'Kippax','Archivo';letter-spacing:.18em;color:var(--label)")}>HOW OTHER FANS CALLED IT</div><div style={s('margin-top:10px')}>{crowd.map((c, i) => <div key={i} style={s('padding:9px 0')}><div style={s("display:flex;justify-content:space-between;font:700 12px/1 'Kippax','Archivo';color:var(--ink)")}><span>{c.s}</span><span>{c.p}</span></div><div style={s('height:6px;background:var(--sand);margin-top:6px')}><span style={{ display: 'block', height: 6, width: c.p, background: '#6CABDD' }} /></div></div>)}</div></div>
    </div>
  );
}
...
function Polls({ st, ft, set, award }: any) {
  ...
  return (
    <div style={s('animation:bgFade .25s ease both;padding:18px 16px 28px')}>
      <div style={s("font:800 30px/.92 'KippaxCondensed','Archivo Black';color:var(--ink)")}>Fan polls</div>
      <div style={s("font:500 13px/1.5 'Kippax','Archivo';color:var(--body);margin-top:8px")}>40 XP per vote. Results update live as the terrace votes.</div>
      {pollDefs.map((p) => { const mine = st.votes[p.id]; const total = Object.values(p.base).reduce((a, b) => a + b, 0) + (mine ? 1 : 0); return (
        <div key={p.id} style={s('margin-top:18px;background:var(--sand);padding:16px')}>
          <div style={s('display:flex;align-items:center;gap:8px')}><span style={{ ...s("font:800 9px/1 'Kippax','Archivo';letter-spacing:.1em;color:#fff;padding:5px 6px"), background: p.tagBg }}>{p.tag}</span><span style={s("font:700 9.5px/1 'Kippax','Archivo';letter-spacing:.1em;color:var(--label);margin-left:auto")}>{(p.votes + (mine ? 1 : 0)).toLocaleString()} VOTES</span></div>
          <div style={s("font:800 22px/1 'KippaxCondensed','Archivo Black';color:var(--ink);margin-top:11px")}>{p.q}</div>
          <div style={s('margin-top:12px;display:flex;flex-direction:column;gap:7px')}>
            {Object.keys(p.base).map((k) => { const v = (p.base as any)[k] + (mine === k ? 1 : 0); const pct = Math.round(v / total * 100); const on = mine === k; return (
              <button key={k} onClick={() => { if (!st.votes[p.id]) { set((sp: FSt) => ({ votes: { ...sp.votes, [p.id]: k } })); award(40, 'POLL VOTED', 'poll'); } }} className="fw" style={s('display:block;width:100%;text-align:left;position:relative;padding:12px 13px;background:var(--ground);overflow:hidden')}>
                ...
                <span style={s('position:relative;display:flex;align-items:center;gap:9px')}><span style={s("flex:1;font:700 13.5px/1.2 'Kippax','Archivo';color:var(--ink)")}>{k}</span><span style={s("font:800 12px/1 'Kippax','Archivo';color:var(--ink);font-variant-numeric:tabular-nums")}>{mine ? pct + '%' : ''}</span>{on && <span style={s("font:800 8.5px/1 'Kippax','Archivo';letter-spacing:.1em;color:#fff;background:var(--panel);padding:4px 5px")}>YOURS</span>}</span>
              </button>
            ); })}
          </div>
        </div>
  ...
  return (
    <div style={s('animation:bgFade .25s ease both')}>
      ...
      <div style={s('padding:16px 16px 0')}>
        <div style={s('display:flex;align-items:center;gap:8px')}><span style={s("font:800 9px/1 'Kippax','Archivo';letter-spacing:.1em;color:#fff;background:#D6202A;padding:5px 6px")}>TONIGHT ONLY</span><span style={s("font:700 9.5px/1 'Kippax','Archivo';letter-spacing:.1em;color:var(--label)")}>FREE COLLECTION AT GATE 4</span></div>
        <div style={s("font:800 28px/.94 'KippaxCondensed','Archivo Black';color:var(--ink);margin-top:11px")}>Semi-final match shirt</div>
...
function Food({ st, set, award, timers }: any) {
  ...
  return (
    <div style={s('animation:bgFade .25s ease both;padding:16px 16px 28px')}>
      ...
      {count > 0 && (
        <div style={s('position:sticky;bottom:0;margin-top:16px;padding:13px 0 4px;background:var(--ground);display:flex;align-items:center;gap:12px')}><div style={s('flex:1')}><div style={s("font:800 9px/1 'Kippax','Archivo';letter-spacing:.12em;color:var(--label)")}>{count} ITEMS</div><div style={s("font:800 17px/1 'Kippax','Archivo';color:var(--ink);margin-top:5px")}>{money(total)}</div></div><button onClick={place} className="fp" style={s("font:800 11.5px/1 'Kippax','Archivo';letter-spacing:.08em;color:var(--on-panel);background:var(--panel);padding:14px 16px")}>{st.foodPlaced ? 'ADD TO ORDER' : 'DELIVER TO 112–J'}</button></div>
      )}
    </div>
  );
}
...
function Photos({ st, award, set }: any) {
  return (
    <div style={s('animation:bgFade .25s ease both;padding:16px 16px 28px')}>
      <div style={s('display:flex;align-items:flex-end;gap:10px')}><div style={s('flex:1')}><div style={s("font:800 28px/.94 'KippaxCondensed','Archivo Black';color:var(--ink)")}>Photo pool</div><div style={s("font:500 12.5px/1.5 'Kippax','Archivo';color:var(--body);margin-top:6px")}>{(st.photos * 134).toLocaleString()} photos from all four stands. Drop yours in for 25 XP.</div></div><button onClick={() => { set((sp: FSt) => ({ photos: Math.min(sp.photos + 3, 18) })); award(25, 'PHOTO ADDED', 'photo'); }} className="fp" style={s("font:800 10.5px/1 'Kippax','Archivo';letter-spacing:.1em;color:var(--on-panel);background:var(--panel);padding:12px 13px;flex:none")}>+ ADD</button></div>
      ...
      <div style={s("font:500 11.5px/1.5 'Kippax','Archivo';color:var(--label);margin-top:12px")}>Tap a tile to drop a real photo in. Anything you add is credited to your seat and enters tonight’s best-photo vote.</div>
    </div>
  );
}
...
function Reads({ st, set }: any) {
  ...
  return (
    <div style={s('animation:bgFade .25s ease both;padding:16px 16px 28px')}>
      {!open && (<>
        <div style={s("font:800 28px/.94 'KippaxCondensed','Archivo Black';color:var(--ink)")}>Reads</div>
        <div style={s("font:500 12.5px/1.5 'Kippax','Archivo';color:var(--body);margin-top:6px")}>Written tonight by people who watch both teams every week.</div>
        <div style={s('margin-top:14px')}>{ARTICLES.map((a, i) => <button key={i} onClick={() => set({ read: i })} className="frow" style={s('display:flex;gap:12px;width:100%;text-align:left;padding:13px 0;border-bottom:1.5px solid var(--hair)')}><span style={s('width:70px;height:70px;flex:none;position:relative;background:var(--sand)')}><Slot id={a.slot} label="Art" /></span><span style={s('flex:1;min-width:0')}><span style={s("display:block;font:800 8.5px/1 'Kippax','Archivo';letter-spacing:.12em;color:#6CABDD")}>{a.kicker}</span><span style={s("display:block;font:700 14.5px/1.25 'Kippax','Archivo';color:var(--ink);margin-top:6px")}>{a.title}</span><span style={s("display:block;font:500 11.5px/1.3 'Kippax','Archivo';color:var(--label);margin-top:5px")}>{a.meta}</span></span></button>)}</div>
      </>)}
      {open && (
        <div style={s('animation:bgFade .25s ease both')}>
          <button onClick={() => set({ read: null })} className="fh" style={s("font:800 10px/1 'Kippax','Archivo';letter-spacing:.14em;color:var(--label)")}><Ms size={14} color="inherit" style={s('vertical-align:-2px')}>arrow_back</Ms> ALL READS</button>
          <div style={s("font:800 8.5px/1 'Kippax','Archivo';letter-spacing:.12em;color:#6CABDD;margin-top:14px")}>{open.kicker}</div>
          <div style={s("font:800 30px/.96 'KippaxCondensed','Archivo Black';color:var(--ink);margin-top:9px")}>{open.title}</div>
          <div style={s("font:500 11.5px/1 'Kippax','Archivo';color:var(--label);margin-top:9px")}>{open.meta}</div>
          ...
          <div style={s('margin-top:14px;display:flex;flex-direction:column;gap:12px')}>{open.body.map((t, i) => <div key={i} style={s("font:400 15px/1.6 'Kippax','Archivo';color:var(--ink)")}>{t}</div>)}</div>
        </div>
      )}
    </div>
  );
...
function Seat({ st, set, go }: any) {
  ...
  return (
    <div style={s('animation:bgFade .25s ease both;padding:16px 16px 28px')}>
      ...
      <button onClick={go('food')} className="fp" style={s("display:block;width:100%;margin-top:16px;padding:15px;background:var(--panel);font:800 12.5px/1 'Kippax','Archivo';letter-spacing:.08em;color:var(--on-panel)")}>ORDER TO THIS SEAT</button>
    </div>
  );
}
  ...
  return (
    <div style={s('animation:bgFade .25s ease both')}>
      ...
      <div style={s('padding:18px 16px 0')}>
        ...
        <div style={s('display:flex;align-items:flex-end;gap:8px;margin-top:10px')}><span style={s("font:800 40px/.82 'KippaxCondensed','Archivo Black';color:var(--ink);font-variant-numeric:tabular-nums")}>{st.xp.toLocaleString()}</span><span style={s("font:600 11.5px/1 'Kippax','Archivo';color:var(--label);padding-bottom:4px")}>/ 2,000 XP</span></div>
        ...
        <div style={s('display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:16px')}>{stats.map((p) => <div key={p.k} style={s('background:var(--sand);padding:13px')}><Ms size={19} color="var(--ink)">{p.icon}</Ms><div style={s("font:800 26px/.9 'KippaxCondensed','Archivo Black';color:var(--ink);margin-top:9px;font-variant-numeric:tabular-nums")}>{p.v}</div><div style={s("font:800 8.5px/1.3 'Kippax','Archivo';letter-spacing:.12em;color:var(--label);margin-top:6px")}>{p.k}</div></div>)}</div>
      </div>
      <div style={s('padding:20px 16px 0')}>{L('TONIGHT’S ACTIVITY')}<div style={s('margin-top:6px')}>{activity.map((a, i) => <div key={i} style={s('display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1.5px solid var(--hair)')}><Ms size={20} color={a.color}>{a.icon}</Ms><span style={s('flex:1;min-width:0')}><span style={s("display:block;font:700 13px/1.2 'Kippax','Archivo';color:var(--ink)")}>{a.t}</span><span style={s("display:block;font:500 11.5px/1.3 'Kippax','Archivo';color:var(--label);margin-top:3px")}>{a.d}</span></span><span style={{ ...s("font:800 10.5px/1 'Kippax','Archivo';letter-spacing:.06em"), color: a.color }}>{a.v}</span></div>)}</div></div>
      ...
      <div style={s('padding:20px 16px 0')}>{L('APPEARANCE')}<div style={s('display:flex;gap:6px;margin-top:10px')}>{themeOpts.map((o) => { const on = st.theme === o.k; return <button key={o.k} onClick={() => set({ theme: o.k })} style={{ ...s('flex:1;display:flex;flex-direction:column;align-items:center;gap:8px;padding:14px 0'), background: on ? 'var(--panel)' : 'var(--sand)', color: on ? 'var(--on-panel)' : 'var(--ink)' }}><Ms size={21} color="inherit">{o.icon}</Ms><span style={s("font:800 10px/1 'Kippax','Archivo';letter-spacing:.1em;color:inherit")}>{o.label}</span></button>; })}</div><div style={s("font:500 11.5px/1.5 'Kippax','Archivo';color:var(--label);margin-top:9px")}>Auto follows your phone’s appearance setting.</div></div>
      <div style={s('padding:20px 16px 26px')}>{L('ALERTS')}<div style={s('margin-top:6px')}>{prefs.map((p) => { const on = st.prefs[p.k]; return <button key={p.k} onClick={() => set((sp: FSt) => ({ prefs: { ...sp.prefs, [p.k]: !sp.prefs[p.k] } }))} style={s('display:flex;align-items:center;gap:12px;width:100%;text-align:left;padding:13px 0;border-bottom:1.5px solid var(--hair)')}><Ms size={20} color="var(--ink)">{p.icon}</Ms><span style={s("flex:1;font:600 13.5px/1.2 'Kippax','Archivo';color:var(--ink)")}>{p.label}</span><span style={{ ...s('width:42px;height:24px;flex:none;border-radius:100px;position:relative;transition:background .25s ease'), background: on ? '#6CABDD' : 'var(--sand2)' }}><span style={{ ...s('position:absolute;top:3px;width:18px;height:18px;border-radius:50%;background:#fff;transition:left .25s cubic-bezier(.2,.8,.2,1)'), left: on ? '21px' : '3px' }} /></span></button>; })}</div><button onClick={go('home')} className="fs" style={s("display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin-top:18px;padding:15px;box-shadow:inset 0 0 0 1.5px var(--hair);font:800 12px/1 'Kippax','Archivo';letter-spacing:.08em;color:var(--ink)")}><Ms size={16} color="var(--ink)">logout</Ms>SIGN OUT</button></div>
    </div>
  );
}
  ...
  return (
    <div style={{ ...s('position:absolute;inset:0;z-index:60;display:flex;flex-direction:column;justify-content:flex-end;background:rgba(0,24,56,.45)'), animation: st.irisOut ? 'bgFadeOut .26s ease both' : undefined }}>
      ...
      <div style={{ ...s('position:relative;height:82%;display:flex;flex-direction:column;background:var(--ground)'), animation: st.irisOut ? 'bgSheetOut .26s cubic-bezier(.32,.72,0,1) both' : 'bgSheet .32s cubic-bezier(.18,.9,.2,1) both' }}>
        ...
        <div style={s('padding:9px 11px 0;display:flex;gap:6px;overflow-x:auto')}>{QA.map(([q, a, stat, statLabel]) => <button key={q} onClick={() => ask(q, a, stat, statLabel)} className="fs" style={s("flex:none;font:700 11px/1 'Kippax','Archivo';color:var(--ink);padding:10px 12px;background:var(--sand);white-space:nowrap")}>{q}</button>)}</div>
        <div style={s('padding:9px 11px 15px;display:flex;gap:8px;align-items:center')}><div style={s('flex:1;padding:12px 14px;background:var(--sand);border-radius:100px')}><input value={st.irisDraft} onChange={(e: any) => set({ irisDraft: e.target.value })} onKeyDown={(e: any) => { if (e.key === 'Enter' && st.irisDraft.trim()) askFree(st.irisDraft); }} placeholder="Ask about tonight’s match…" style={s("width:100%;font:500 13.5px/1.2 'Kippax','Archivo';color:var(--ink)")} /></div><button onClick={() => { if (st.irisDraft.trim()) askFree(st.irisDraft); }} style={s("width:44px;height:44px;flex:none;background:var(--panel);border-radius:50%;display:flex;align-items:center;justify-content:center;font:800 16px/1 'Kippax','Archivo';color:var(--on-panel)")}>↑</button></div>
      </div>
    </div>
  );
}
